...
下面的是SUMO文件使用的
'''
def set_visible_area_in_sumo(element, axes):
    min_x = 10e9
    min_y = 10e9
    max_x = -10e9
    max_y = -10e9

    for item in element.findall("location"):
        convBoundary=item.get("convBoundary")
        convBoundary_str_list=convBoundary.split(",",-1)
        min_x=float(convBoundary_str_list[0])
        min_y=float(convBoundary_str_list[1])
        max_x=float(convBoundary_str_list[2])
        max_y=float(convBoundary_str_list[3])

    axes.set_aspect('equal', adjustable='box')
    axes.set_xlim([min_x - 10, max_x + 10])
    axes.set_ylim([min_y - 10, max_y + 10])

def get_lane_x_list_and_y_list(element):
    x_list = list()
    y_list = list()

    shape = element.get("shape")  # 例如shape="-8.75,75.50 -10.11,65.98 -14.19,59.19 -20.98,55.11 -30.50,53.75"
...
def draw_map_with_SUMO(filename, axes):
    assert isinstance(axes, matplotlib.axes.Axes)
    ...
    set_visible_area_in_sumo(e, axes)

    type_dict = dict(color="black", linewidth=1, zorder=10)

    for node in e.findall("edge"):
        for tag in node.findall("lane"):
            x_list, y_list = get_lane_x_list_and_y_list(tag)
            plt.plot(x_list, y_list, **type_dict)
