...
def read_tracks_per_stamp(model_out_put_dict,current_time_stamp):
    track_dict = dict()

    for key in model_out_put_dict.keys():
        track_id = key
        track = Track(track_id)
        track.agent_type = model_out_put_dict[key]['type']
        track.length = model_out_put_dict[key]['length']
        track.width = model_out_put_dict[key]['width']
        track.foe_vehicle_id=model_out_put_dict[key]['foe_vehicle']
        # 下面这两行起始结束时间都不要
        # track.time_stamp_ms_first = int(row[KeyEnum.time_stamp_ms])
        # track.time_stamp_ms_last = int(row[KeyEnum.time_stamp_ms])

        ms = MotionState(current_time_stamp)
        ms.x = float(model_out_put_dict[key]['x'])
        ms.y = float(model_out_put_dict[key]['y'])
        ms.vx = None
        ms.vy = None
        ms.psi_rad = float(model_out_put_dict[key]['psi_rad'])
        track.motion_states[ms.time_stamp_ms] = ms
        track_dict[track_id] = track
